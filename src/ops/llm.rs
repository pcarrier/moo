...
pub async fn stream_chat(
    ...
    let client = match reqwest::Client::builder().build() {
        Ok(c) => c,
        Err(e) => {
            return transport_error(&pool, &bundle, 0, format!("client: {e}"), Value::Null).await;
        }
    };
    ...
    let resp = match req.send().await {
        Ok(r) => r,
        Err(e) => {
            return transport_error(&pool, &bundle, 0, format!("send: {e}"), Value::Null).await;
        }
    };
    let status = resp.status().as_u16();
    if status >= 400 {
        let body_text = resp.text().await.unwrap_or_default();
        return transport_error(&pool, &bundle, status, body_text, Value::Null).await;
    }
    ...
    match call_harness(
        &pool,
        &bundle,
        json!({ "command": "llm-stream-finalize", "state": accumulator, "status": status }),
    )
    .await
    {
        Ok(v) => v,

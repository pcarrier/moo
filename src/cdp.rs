...
fn run_inspector_thread(
    ...
    loop {
        ...
        let session_resources = SessionResources {
            ...
            script_url: &format!("http://{advertised_host}/harness.js"),
            source_map_url: map_url,
            source_map_content: map_resource.as_ref().map(|(_, content)| content.as_str()),
        };
        let session_context = SessionContext {
            inspector: &inspector,
            shared: &shared,
            shared_ptr,
            inspect_rx: &inspect_rx,
            active_target: &active_target,
        };
        run_session(&mut isolate, pending, session_context, session_resources);
    }
}

struct SessionResources<'a> {
    bundle_source: &'a str,
    script_url: &'a str,
    source_map_url: Option<&'a str>,
    source_map_content: Option<&'a str>,
}

struct SessionContext<'a> {
    inspector: &'a V8Inspector,
    shared: &'a Shared,
    shared_ptr: *const Shared,
    inspect_rx: &'a Receiver<InspectRequest>,
    active_target: &'a Arc<Mutex<Option<TargetSpec>>>,
}
...
fn run_session(
    ...
    resources: SessionResources<'_>,
) {
    {
        let mut cdp_resources = session.shared.cdp_resources.borrow_mut();
        cdp_resources.clear();
        cdp_resources.insert(
            resources.script_url.to_string(),
            CdpResource {
                content: resources.bundle_source.to_string(),
                content_type: "application/javascript; charset=utf-8",
            },
        );
        if let (Some(url), Some(content)) = (resources.source_map_url, resources.source_map_content)
        {
            cdp_resources.insert(
                url.to_string(),
                CdpResource {
    ...
    *session.shared.default_context.borrow_mut() = Some(context_global);
    ...
    .to_string();
    session.inspector.context_created(
        context,
        CONTEXT_GROUP_ID,
    ...
    // context internally for each Runtime/Debugger command.
    {
        ...
        if !resources.bundle_source.is_empty() {
            eval_bundle_in_scope(
                &mut scope,
                resources.bundle_source,
                resources.script_url,
                resources.source_map_url,
            );
        }
    }

    *session.shared.out_tx.borrow_mut() = Some(pending.out_tx.clone());
    let channel = Channel::new(Box::new(OutboundChannel {
        out_tx: pending.out_tx,
        shared: session.shared_ptr,
    }));
    let inspector_session = session.inspector.connect(
        CONTEXT_GROUP_ID,
        channel,
        StringView::empty(),
    ...
    *session.shared.default_context.borrow_mut() = None;
}

...
- keeps chats, pointers, objects, and RDF-style facts in a local SQLite store
- serves a local web shell for chatting, browsing memory, and managing MCP servers
- exposes host ops for files, processes, HTTP, env, pointers, facts, and more
...
Use the checked-in bump script to update the package version in Cargo, Nix, and embedded client metadata:

```sh
scripts/bump-version.mjs 0.1.1
```

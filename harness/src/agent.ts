...
import { providerConfiguredCredential } from "./commands/llm_auth";
...
async function selectedChatFact(chatId: string, predicate: string): Promise<string | null> {
  ...
  const rows = await moo.facts.match({ ref: c.facts, ...{
    graph: c.graph,
    subject: `chat:${chatId}`,
    predicate,
    limit: 1,
...
export function estimateRawUsage(
  ...
  const prompt = Array.isArray(messages) ? estimateTokens(messages) : (Number.isFinite(fallbackPrompt) ? Math.max(0, fallbackPrompt) : 0);
...
export async function loadPayloadJSON(
  ...
  const rows = await moo.facts.match({ ref: factsRef, ...{
    graph,
    subject,
    predicate,
    limit: 1,
...
async function finishRunJSStep(
  ...
  const resultHash = result
    ...
    : error
      ...
      : null;
  const statusRows = await moo.facts.match({ ref: c.facts, ...{ graph: c.graph, subject: stepId, predicate: "agent:status" } });

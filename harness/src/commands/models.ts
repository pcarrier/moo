...
async function getChatFact(chatId: string, predicate: string): Promise<string | null> {
  const rows = await moo.facts.match({ ref: chatFactsRef(chatId), ...{
...
async function setChatFact(chatId: string, predicate: string, value: string | null): Promise<void> {
  ...
  const existing = await moo.facts.match({ ref: ref, ...{ graph, subject, predicate } });
  ...
  await moo.facts.swap({ ref: ref, removes, adds });
}
...
export async function lastChatModel(): Promise<string | null> {
  const stored = String((await moo.refs.get(LAST_MODEL_REF)) ?? "").trim();
  ...
  const provider = normalizeProvider(await moo.refs.get(LAST_PROVIDER_REF));
...
export async function rememberChatModel(model: string | null): Promise<void> {
  ...
  const modelName = parsed.model || null;
  const provider = modelName ? (parsed.provider || inferProviderNameForModel(modelName)) : null;
  if (modelName) await moo.refs.set(LAST_MODEL_REF, modelName);
  else await moo.refs.delete(LAST_MODEL_REF);
  if (provider) await moo.refs.set(LAST_PROVIDER_REF, provider);
  else await moo.refs.delete(LAST_PROVIDER_REF);
}

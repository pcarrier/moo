...
function factTimestamp(value: unknown): number {
  ...
  const literal = parseFactLiteral(raw);
  if (literal == null) return 0;
  const numeric = finiteNumber(literal);
...
function factString(value: unknown): string | null {
  ...
  if (!trimmed) return "";
  return parseFactLiteral(trimmed) ?? value;
}

function firstPresent(...values: unknown[]): string | null {
  for (const value of values) {
    if (value !== undefined && value !== null) return factString(value);
  }
  return null;
}
...
function diffPayloadSummary(
  ...
  return {
    ...
    // The timeline and Trails sidebar only need the patch text and stats for
    // collapsed rows. Full before/after snapshots can dominate initial chat
    // loads; leave them in the object store for explicit object/sidebar reads.
    diff: value?.diff || "",
    stats: value?.stats,
    hash: value?.hash || payloadHash,
    store: value?.store,
    graph: value?.graph,
    action: value?.action,
    count: value?.count,
...
async function loadTrailEntryRows(c: ReturnType<typeof chatRefs>, limit = 0) {
  return selectRows(
    c,
    `?entry rdf:type agent:TrailEntry . ?entry agent:kind ?kind . ?entry agent:createdAt ?at .
     optional { ?entry agent:title ?title . }
     optional { ?entry agent:body ?body . } optional { ?entry agent:summary ?summary . }`,
    "?entry ?kind ?at ?title ?body ?summary",
    limit,
  );
}
...
function trailRowToTimelineItem(row: any) {
  const summary = factString(row["?summary"]);
  const body = firstPresent(row["?body"], row["?summary"]);
  return {
    type: "trail",
    id: row["?entry"],
    kind: row["?kind"] || "agent:Summary",
    at: factTimestamp(row["?at"]),
    title: factString(row["?title"]),
    body,
    summary,
  };
}
...
export async function describeCommand(input: Input) {
  ...
  const mode = input.mode === "update" ? "update" : "snapshot";
  if (mode === "update") {
    ...
    if (knownHeadMatches && knownTimelineMatches && knownCompactionMatches) {
      return { ok: true, value: { mode, overview, changed: false } };
    }
    const sinceAt = Number(input.sinceAt ?? 0);
    if (!Number.isFinite(sinceAt) || sinceAt <= 0) {
      return { ok: true, value: { mode, overview, changed: true } };
    }
    const snapshot = await loadTimelineSnapshot(chatId, {
      limit: input.limit,
      sinceAt,
      includeTrail: false,
    });
...
async function loadTimelineSnapshot(
  chatId: string,
  options: { limit?: unknown; sinceAt?: unknown; includeTrail: boolean },
) {
  const c = chatRefs(chatId);
  ...
  return {
    ...
    timeline: {
      ...
      limit: timelineLimit,
      ...(Number.isFinite(sinceAt) && sinceAt > 0 ? { sinceAt } : {}),
    },
    trail: {

...
async function removeDeletedAt(ref: string, graph: string, stepId: string, txn: any) {
  const existing = await moo.facts.match(ref, {
    graph,
    subject: stepId,
    predicate: "agent:deletedAt",
  });
  for (const [g, s, p, o] of existing) txn.remove(g, s, p, o);
}
...
export async function messageDeleteCommand(input: Input) {
  ...
  const c = found.value;
  const deletedAt = String(await moo.time.now());
  await moo.facts.update(c.facts, async (txn) => {
    await removeDeletedAt(c.facts, c.graph, stepId, txn);
    txn.add(c.graph, stepId, "agent:deletedAt", deletedAt);
  });

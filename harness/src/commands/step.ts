...
export async function enqueueCommand(input: Input) {
  ...
  if (kind === "agent:ShellCommand") {
    ...
    payloadHash = await moo.objects.put(
      kind,
      JSON.stringify({
        cmd: p.cmd,
        args: p.args || [],
    ...
    payloadHash = await moo.objects.put(kind, JSON.stringify(input.payload));
  }
...
export async function tickCommand(input: Input) {
  ...
  const kindRows = await moo.facts.match(c.facts, {
    graph: c.graph,
    subject: stepId,
  ...
  try {
    if (kind === "agent:ShellCommand") {
      ...
      result = await moo.proc.run(cmd, args || [], { cwd: wt, stdin });
      resultHash = await moo.objects.put(
        "agent:ToolResult",
        JSON.stringify({ kind, cmd, args, cwd: wt, ...result }),
      );
  ...
  await moo.facts.update(c.facts, (txn) => {
    ...
    if (result) txn.add(c.graph, stepId, "agent:exitCode", String(result.code));
    if (errorMessage) txn.add(c.graph, stepId, "agent:error", errorMessage);
    if (leaseId) txn.remove(c.graph, stepId, "agent:lease", leaseId);
  });
...
export async function submitCommand(input: Input) {
  ...
  const statusRows = await moo.facts.match(c.facts, {
    graph: c.graph,
    subject: requestId,
    predicate: "ui:status",
    limit: 1,
  ...
  const respondedAt = await moo.time.now();
  const respPayload = await moo.objects.put(
    "ui:Response",
    JSON.stringify({ values, at: respondedAt, ...(cancelled ? { cancelled: true } : {}) }),
  );
  const respId = await moo.id.new("uires");

  await moo.facts.update(c.facts, (txn) => {
    txn.add(c.graph, respId, "rdf:type", "ui:InputResponse");
    txn.add(c.graph, respId, "ui:respondsTo", requestId);
    txn.add(c.graph, respId, "ui:payload", respPayload);
    txn.add(c.graph, respId, "ui:createdAt", String(respondedAt));
    txn.remove(c.graph, requestId, "ui:status", "ui:Pending");
    txn.add(c.graph, requestId, "ui:status", cancelled ? "ui:Cancelled" : "ui:Done");
  });
...
export async function setChatOngoing(chatId: string, ongoing: boolean) {
  if (ongoing) await moo.refs.set(chatOngoingRef(chatId), String(await moo.time.now()));
...
export async function stepPreludeCommand(input: Input) {
  ...
  // Slash commands are dispatched inline (synchronous). They may invoke
  // shell commands or compaction, but they don't run the agent loop. The
  // driver returns immediately.
  if (message.startsWith("/")) {
    await respondTo(chatId, message);
...
export async function stepContinueToolCallsCommand(input: Input) {
  ...
  for (let i = 0; i < toolCalls.length; i++) {
    ...
    const exec = await executeToolCall(chatId, tc, usedModel);
...
export async function runJsToolCommand(input: Input) {
  ...
  try {
    const exec = await runToolCall(chatId, input.toolCall, input.model ?? input.state?.usedModel ?? null);
    return {
      ok: true,
      value: {
...
export async function subagentFinalCommand(input: Input) {
  ...
  if (!latest) return { ok: true, value: { text: "" } };
  const payload = await loadPayloadJSON(c.facts, c.graph, latest.step);
  return { ok: true, value: { text: String(payload?.value?.text ?? "") } };
}
...
export async function restartOngoingCommand() {
  ...
  for (const c of chats) {
    ...
    if (marked || c.status === "agent:Running" || c.status === "agent:Queued") {
...
export async function stepHandleLlmCommand(input: Input) {
  ...
  const normalizedUsage = normalizeUsage(llmResult.usage);
  if (llmResult.ok) {

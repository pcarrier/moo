import { moo } from "../moo";
import { allFactStores, type Input } from "./_shared";
...
export async function dumpCommand(input: Input) {
  ...
  const store = cleanOptionalString(input.store) ?? cleanOptionalString(input.ref) ?? (chatId ? "chat/" + chatId + "/facts" : null);
  ...
  const stores = store ? [store] : await allFactStores();
  const rows: DumpQuad[] = [];
  for (const s of stores) {
    rows.push(...((await moo.facts.match({ store: s, ...pattern })) as DumpQuad[]));
  }

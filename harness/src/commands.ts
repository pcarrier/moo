...
const COMMANDS: Record<string, CommandHandler> = {
  ...
  "memory-patch": memoryPatchCommand,
...
function mergeArgv(command: string, rest: string[]): Input {
  ...
  switch (command) {
    ...
    case "chat-fork":
      out.chatId = rest[0];
      out.step = rest[1];
      if (rest[2]) out.forkChatId = rest[2];
      return out;
    case "graph-rm":
      out.graph = rest[0];
      return out;
    ...
    case "llm-auth-get":
      return out;
    case "llm-auth-save":
      try { Object.assign(out, JSON.parse(rest.join(" ") || "{}")); } catch {}
      return out;
    case "llm-auth-oauth-start":
    case "llm-auth-oauth-complete":
    case "llm-auth-oauth-device-start":
    case "llm-auth-oauth-device-poll":
    case "llm-auth-oauth-logout":
      return out;
    case "schema":
      if (rest[0]) out.chatId = rest[0];
      return out;
    ...
    case "mcp-call":
      out.serverId = rest[0];
      out.name = rest[1];
      try { out.arguments = JSON.parse(rest.slice(2).join(" ") || "{}"); } catch { out.arguments = {}; }
      return out;
    case "ui-bundle":
    case "ui-remove":
      out.uiId = rest[0];

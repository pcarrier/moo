...
export async function buildSystemPrompt(chatId: string): Promise<string> {
  ...
  return [
    ...
    "edits: no `moo.fs.patch`; before brittle replacements, verify target text with `rg`/`fs.readLines`; use targeted `fs.write` updates or CLI/editor commands, then reread when context may have changed.",

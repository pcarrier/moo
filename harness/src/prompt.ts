...
export async function buildSystemPrompt(chatId: string): Promise<string> {
  ...
  return [
    ...
    "  RDF terms: use exact terms returned by match/query for delete/retract; don't add/remove quotes. Use moo.term.string({s:'x'}) for literals that look like IRIs/numbers/bools, moo.term.iri({uri:'prefix:Local'}) for uppercase/ambiguous CURIEs.",
    ...
    "  e.g. assert({subject:'user:me',predicate:'prefers',object:'tool:vim'}); assert({subject:'user:me',predicate:'bio',object:moo.term.string({s:'Hi: vim'})}).",

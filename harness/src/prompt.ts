...
export async function buildSystemPrompt(chatId: string): Promise<string> {
  ...
  return [
    ...
    "fs: read(path:string)→Promise<string>; readLines(path:string,ranges:[number,number][],opts?:{numbered?:boolean})→Promise<string[]>; write(path:string,content:string)→Promise<void>; list(path:string)→Promise<string[]>; glob(pattern:string)→Promise<string[]>; stat(path:string)→Promise<{kind:string,size:number,mtime:number}|null>; canonical(path:string)→Promise<string>; exists(path:string)→Promise<boolean>; ensureDir(path:string)→Promise<void>. Relative paths resolve under scratch.",

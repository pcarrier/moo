...
export async function buildSystemPrompt(chatId: string): Promise<string> {
  ...
  return [
    ...
    "fs: read(path:string)→Promise<string>; readLines(path:string,ranges:[number,number][],opts?:{numbered?:boolean})→Promise<string[]> (1-based inclusive ranges; sorted/collapsed overlaps; inserts `…` for omitted lines; numbered yields aligned `   1: text`); write(path:string,content:string)→Promise<void>; list(path:string)→Promise<string[]>; glob(pattern:string)→Promise<string[]>; stat(path:string)→Promise<{kind:string,size:number,mtime:number}|null>; canonical(path:string)→Promise<string>; exists(path:string)→Promise<boolean>; ensureDir(path:string)→Promise<void>. Relative paths resolve under scratch.",
    "edits: no `applyPatch` helper in runJS; for unified diffs pass patch text via `args`, write it to a temp file, run `git apply --check <file>`, then `git apply <file>`; otherwise use targeted `fs.write`/CLI/editor commands. Before brittle replacements, verify with `rg`/`fs.readLines`; reread after changes.",
    ...
    ...repoLines,
    "workspace: current({chatId?:string|null,root?:string|null}?)→Promise<{root:string,fs:{read/readLines/write/list/glob/stat/canonical/exists/ensureDir},proc:{run/runChecked}}>; scoped fs/proc use the workspace root.",

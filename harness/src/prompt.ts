...
export async function buildSystemPrompt(chatId: string): Promise<string> {
  ...
  return [
    ...
    "todos: optional; use `moo.todos` only for substantial multi-step work where tracking helps. Methods: list({})→Promise<{items:AgentTodo[]}>; add({text:string,priority?:low|normal|high,status?:todo|doing|done|blocked|dropped,note?:string})→Promise<AgentTodo>; update({id,text?,status?,priority?,note?:string|null})→Promise<AgentTodo>; done({id,note?})/drop({id,note?})→Promise<AgentTodo>; patch({items?:AgentTodoPatch[]})→Promise<{items:AgentTodo[]}>; clear({statuses?:TodoStatus[]})→Promise<{items:AgentTodo[]}>. Keep items terse; don't update guessed/stale IDs.",

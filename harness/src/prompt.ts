...
export async function buildSystemPrompt(chatId: string): Promise<string> {
  ...
  return [
    ...
    "workflows: workflow(id,w=>w.flow(...)) emits JSON IR; workflows.save({definition}), list(), inspect(id), runs(filter?), waiting(filter?), inspectRun(runId), start(id,{input?,state?,chatId?,autoResume?}), resume(runId), submit(runId,stepPath,value), cancel/retry/fork/linkChat/unlinkChat/renderMermaid. Runs are pointer+RDF backed and independent of chats.",
    ...skillsLines,
    ...
    "fs: read(path:string)→Promise<string>; write(path:string,content:string)→Promise<void>; list(path:string)→Promise<string[]>; glob(pattern:string)→Promise<string[]>; stat(path:string)→Promise<{kind:string,size:number,mtime:number}|null>; canonical(path:string)→Promise<string>; exists(path:string)→Promise<boolean>; ensureDir(path:string)→Promise<void>. Relative paths resolve under scratch.",
    ...
    ...repoLines,
    "workspace: current({chatId?:string|null,root?:string|null}?)→Promise<{root:string,fs:{read/write/list/glob/stat/canonical/exists/ensureDir},proc:{run/runChecked}}>; scoped fs/proc use the workspace root.",

...
const todoRegion = timeline.slice(
  timeline.indexOf("function OngoingTodos"),
  timeline.indexOf("function PendingList"),
);
const todoDiffRegion = timeline.slice(
  timeline.indexOf("function TodoDiffBody"),
...
describe("timeline TODO UI", () => {
  test("renders ongoing TODOs between the timeline and queued/input messages", () => {
    ...
    const ongoingTodos = timeline.indexOf(
    ...
    expect(timeline).toContain("<TodoMarkdownInline");
    expect(timeline).toContain('className="ongoing-todo-text"');
    ...
    expect(timeline).toContain("<TodoMarkdownBlock");
    expect(timeline).toContain("className={`${props.className} todo-note`}");
    ...
    expect(todoRegion).toContain('"only-done":');
    expect(todoRegion).toContain("!showDone() &&");
    expect(todoRegion).toContain("visibleItems().length === 0 &&");
    expect(todoRegion).toContain("doneItems().length > 0");
...
.ongoing-todo-details {
  ...
  flex: 1 1 auto;
    ...
    expect(css).not.toContain(`.ongoing-todo-details {
  flex: 0 1 auto;
  max-inline-size: 35%;
}`);
  ...
  test("renders TODO diffs in the timeline without diff chrome", () => {
    ...
    expect(timeline).toContain("props.item as FileDiffItem | MemoryDiffItem | TodoDiffItem");
    expect(timeline).toContain(
  ...
  test("renders TODO diffs as semantic changes", () => {
    ...
    expect(todoDiffRegion).toContain("<TodoMarkdownInline");
    expect(todoDiffRegion).toContain('className="todo-diff-text"');
    expect(todoDiffRegion).toContain("content={todoLabel(item())}");
    expect(todoDiffRegion.indexOf('className="todo-diff-text"')).toBeLessThan(
      todoDiffRegion.indexOf("<TodoMetaBubbles item={item()} />"),
    );
    expect(todoDiffRegion).toContain('<Show when={change.kind !== "removed"}>');
    expect(todoDiffRegion).toContain('className="todo-diff-previous-text"');
    expect(todoDiffRegion).toContain("content={`was: ${todoLabel(previous()!)}`}");
    expect(timeline).not.toContain("No TODO changes");
    expect(timeline).not.toContain("todo-diff-fallback");
    expect(todoDiffRegion).toContain("<TodoNote");
    expect(todoDiffRegion).toContain("item={item()}");
    expect(todoDiffRegion).toContain('className="todo-diff-details"');
    ...
    expect(css).toContain(
      "--bubble-todo-diff: color-mix(in srgb, orchid 16%, transparent);",
    );
    expect(css).toContain(
    ...
    expect(css).toContain(
      ".todo-change-updated .todo-diff-action {\n  color: var(--accent-fg);\n}",
    );
    expect(css).toContain(
      ".todo-status-doing .todo-diff-text {\n  font-weight: 700;\n}",
    );
    expect(css).toContain(
      ".todo-status-dropped .todo-diff-action {\n  color: var(--danger-fg, #cf222e);\n}",
    );
  ...
  test("scopes ongoing TODOs to the selected chat", () => {
    ...
    const todoDiffEnd = state.indexOf(
    ...
    expect(
      todoDiffBlock.indexOf("applyTodosForChat(ev.chatId, ev.todos)"),
    ).toBeLessThan(todoDiffBlock.indexOf("if (cid && ev.chatId === cid)"));
    expect(todoDiffBlock).toContain(

...
const PRISM_LANGUAGE_BY_INFO_STRING: Record<string, string> = {
  bash: "bash",
  css: "css",
  html: "markup",
...
type RenderContext = Required<HjsonFormatOptions> & {
  html: boolean;
  seen: WeakSet<object>;
...
export function highlightMarkdownCode(text: string, language: string | null | undefined): string {
  ...
  return cachedHighlight("md:" + lang + "\0" + text, text.length, () => {
    ...
    const prismLanguage = PRISM_LANGUAGE_BY_INFO_STRING[lang];
...
export function highlightByPath(text: string, path: string): string {
  ...
  const prismLanguage = PRISM_LANGUAGE_BY_EXTENSION[extension];
  if (prismLanguage) return highlightCodeRecursive(text, prismLanguage, 0);
...
function highlightDiffLine(line: string, language: string | null): string {
  ...
  const bodyHtml = highlightBody && language ? highlightCodeRecursive(body, language, 0) : escapeHtml(body);
...
function prismLanguageForPath(path: string): string | null {
  const extension = fileExtension(path);
  return extension ? PRISM_LANGUAGE_BY_EXTENSION[extension] || null : null;
}
...
export function highlightGenericCode(text: string, language?: string): string {
  if (language) {
    const prismLanguage = PRISM_LANGUAGE_BY_INFO_STRING[normalizeCodeLanguage(language)] || normalizeCodeLanguage(language);

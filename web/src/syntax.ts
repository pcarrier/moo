  ...
  const fmt = (v: unknown, depth: number): string => {
    ...
    try {
      ...
      const lines = entries.map(([key, val]) => {
        ...
        const sep = isHjsonMultilineString(val) ? ":\n" + pad : ": ";
        return pad + formatHjsonKey(key) + sep + formatted;
      });
      return "{\n" + lines.join("\n") + "\n" + endPad + "}";
    } finally {
      seen.delete(v);
...
export function highlightJson(text: string): string {
  ...
  while (i < text.length) {
    ...
    if (text.startsWith("'''", i)) {
      ...
      const tok = end === -1 ? text.slice(i) : text.slice(i, end + 3);
      html += '<span class="json-str">' + escapeHtml(tok) + '</span>';
      i += tok.length;
      continue;
    }
    if (ch === "\"" || ch === "\'") {

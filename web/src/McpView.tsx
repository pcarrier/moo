...
export function McpView(props: { bag: Bag; onToggleSidebar?: () => void }) {
  ...
  async function refresh() {
    setBusy(true);
    setMessage(null);
    const listed = await api.mcpList();
    if (!listed.ok) {
    ...
    await Promise.all(nextServers.map(async (s) => {
      if (!s.oauth) return;
      const status = await api.mcpOAuthStatus(s.id);
      if (status.ok) statuses[s.id] = status.value.status.authenticated;
    }));
    setAuth(statuses);
    const allTools = await api.mcpTools();
  ...
  async function save(options: { login?: boolean } = {}) {
    ...
    try {
      ...
      const saved = await api.mcpSave(config);
      ...
      if (options.login && saved.value.server.oauth) {
        const started = await api.mcpOAuthStart(saved.value.server.id, window.location.origin);
        if (!started.ok) {
  ...
  async function login(id: string) {
    setBusy(true);
    setMessage(null);
    const started = await api.mcpOAuthStart(id, window.location.origin);
    setBusy(false);
    if (!started.ok) {
      setMessage(started.error.message);
      return;
  ...
  async function logout(id: string) {
    setBusy(true);
    const removed = await api.mcpOAuthLogout(id);
  ...
  async function remove(id: string) {
    ...
    setBusy(true);
    const removed = await api.mcpRemove(id);
    if (!removed.ok) setMessage(removed.error.message);
    else {
      if (selected() === id) {
        setSelected(null);
    ...
    <div class="chat-shell mcp-dashboard-view">
      <section class="main mcp-view">
        <header class="conv-header">
          ...
          <button class="collapse-btn" title="back to chat" onClick={() => bag.showChat()}>
            ←
          </button>
          ...
          <button class="icon-btn" title="refresh MCP servers" disabled={busy()} onClick={refresh}>
            ↻
          </button>
          ...
          <section class="mcp-card">
            ...
            <Show when={tools().length > 0} fallback={<div class="empty">No tools discovered.</div>}>
              <ul class="mcp-tool-list">
                <For each={tools()}>{(tool) => (
                  <li>
                    <code>moo.mcp.{tool.serverId || tool.server}.{tool.name}()</code>
                    <Show when={tool.description}><span>{tool.description}</span></Show>
                  </li>
                )}</For>
              </ul>
            </Show>

...
function expectedChatWorktreePath(chat: Pick<ChatSummary, "chatId" | "path"> | null | undefined): string | null {
  ...
  return root.replace(/\/+$/, "") + "/.moo/" + id.replace(/^\/+/, "");
}

function withExpectedChatWorktreePath(chat: ChatSummary): ChatSummary {
  return chat.worktreePath == null
    ? { ...chat, worktreePath: expectedChatWorktreePath(chat) }
    : chat;
}

function withExpectedChatWorktreePaths(chats: ChatSummary[]): ChatSummary[] {
...
export function createState() {
  ...
  function setRightSidebarW(width: number | string) {
    ...
    setRightSidebarLayoutByChat((prev) => {
      ...
      return {
        ...prev,
        [RIGHT_SIDEBAR_DEFAULT_LAYOUT_ID]: { ...defaults, width: next },
  ...
  createEffect(() => localStorage.setItem(SIDEBAR_KEY, sidebarW()));
...
${r.value.url}`);
  ...
  async function interruptAgent(options: { resumeQueued?: boolean; offerResume?: boolean } = {}) {
    ...
    // do not wait for the interrupt request/step-end round trip.
  ...
  // view (the sidebar fact count tolerates staleness).
  const heartbeat = window.setInterval(async () => {
    await refreshTimeline();
    if (!thinking() && view() === "facts") {

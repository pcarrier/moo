...
describe("timeline fork action", () => {
  test("navigates to a seeded fork before the backend copy finishes", () => {
    ...
    const forkEnd = stateSource.indexOf(
    ...
    expect(forkBlock).toContain(
      "pendingChatCreations.set(requestedChatId, creation);",
    );
    expect(forkBlock).toContain("await selectChat(requestedChatId, false);");
    expect(forkBlock).toContain('const r = await api("chat-fork", {');
    expect(forkBlock).toContain("chatId: id as ChatId,");
    expect(forkBlock).toContain("step: step as StepId,");
    expect(forkBlock).toContain("forkChatId: requestedChatId,");
    expect(
      forkBlock.indexOf("await selectChat(requestedChatId, false);"),
    ).toBeGreaterThan(
      forkBlock.indexOf("pendingChatCreations.set(requestedChatId, creation);"),
    );
    expect(
      forkBlock.indexOf("pendingChatCreations.set(requestedChatId, creation);"),
    ).toBeGreaterThan(forkBlock.indexOf("const creation = (async () => {"));
  });
  ...
  test("waits for pending fork creation before hydration refreshes", () => {
    ...
    const selectEnd = stateSource.indexOf(
    ...
    expect(
      selectBlock.indexOf("if (!(await waitForChatCreation(id))) return;"),
    ).toBeLessThan(selectBlock.indexOf("await refreshTimeline();"));
  });

  test("passes the optimistic fork id to chat-fork", () => {
    expect(stateSource).toContain('const r = await api("chat-fork", {');
    expect(stateSource).toContain("chatId: id as ChatId,");
    expect(stateSource).toContain("step: step as StepId,");
    expect(stateSource).toContain("forkChatId: requestedChatId,");
  });
});

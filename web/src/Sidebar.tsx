...
import { highlightAuto, highlightByPath } from "./syntax";
...
function EntryDiffBadge(props: { entry: DiffCountSource }) {
  const stats = () => entryDiffCount(props.entry);
  return (
    <span
      class="fs-entry-delta"
      title={entryDiffTitle(props.entry)}
      aria-label={entryDiffTitle(props.entry)}
    >
      <span class="fs-entry-added">+{stats().additions}</span>
      <span class="fs-entry-removed">−{stats().deletions}</span>
    </span>
  );
}
...
export function RepoFilePreview(props: {
  ...
  return (
    <section class="repo-file-preview" aria-label="opened repository file">
      ...
      <Show when={props.file.loading && !previewPath()}>
      ...
      <Show when={!props.file.error || previewPath()}>
        <div class="repo-file-meta repo-file-toolbar">
          ...
          <Show when={previewFile().changed}>
            <EntryDiffBadge entry={previewFile()} />
          </Show>
          <Show when={props.file.loading}>
            <span class="repo-file-loading-inline">refreshing…</span>
          </Show>
            ...
            <Show
              ...
              fallback={
                  ...
                  fallback={
                    ...
                    >
                      <DiffDetailTab
                        bag={props.bag}
                        tab={
                          tab() as Extract<RightSidebarTab, { kind: "diff" }>
                        }
                      />
                    </Show>
                  }
                >
                  <StorePreviewTab
                    store={
                      (tab() as Extract<RightSidebarTab, { kind: "store" }>)
                        .store
                    }
                    onClose={() =>
                      void props.bag.closeRightSidebarTab(tab().id)
...
function BrowserTab(props: {
  ...
  const timelineDiffStats = createMemo(() => {
    ...
    for (const diff of mergedFileDiffs(trailSourceItems(props.bag))) {
      addBrowserDiffStats(
        stats,
        diff.path,
        diff.stats?.added ?? 0,
  ...
  createEffect(() => {
    ...
    void api.fs.read(path, rootPath()).then((r) => {
      ...
      <span class="agent-trail-card">
        <span class="agent-trail-title-line">
          ...
          <Show when={props.item.stats}>
            {(stats) => (
              <span
                class="right-diff-stats agent-trail-diff-stats"
                aria-label={diffStatLabel({ stats: stats() })}
              >
...
function subagentTimelineItem(item: StepItem): AgentTrailItem | null {
  ...
  const duration =
    typeof info.result?.durationMs === "number"
          ...
          {(item) => {
            ...
            return (
              ...
              >
                ...
                <span
                  class="right-diff-stats"
                  aria-label={factStatLabel(stats)}
                >
                  <span class="right-diff-added">+{stats.added}</span>
                  <span class="right-diff-removed">−{stats.removed}</span>
                </span>
...
function DiffDetailTab(props: {
  ...
  const item = createMemo(() => {
    if (props.tab.scope === "history") {
      return (
        mergedFileDiffs(props.bag.trail()).find((candidate) =>
  ...
  const scopeLabel = () =>
    props.tab.scope === "history" ? "Total change" : "Individual change";
  ...
  const renderedSource = createMemo(() =>
    highlightByPath(afterContent(), path()),
  );
  createEffect(() => {
  ...
  return (
    <section
      ...
      aria-label={`${scopeLabel()} for ${displayPath()}`}
    >
...
function htmlPreviewAssetBasePath(
  path: string,
  assetRootPath?: string | null,
): string | null {
  const normalizedPath = normalizeRepoPath(path).replace(/\/+$/, "");
...
export function Sidebar(props: { bag: Bag; onNavigate?: () => void }) {
  ...
  return (
    <aside class="sidebar">
        ...
        >
          ...
          <Show when={hiddenActiveChats() > 0}>
            <li class="chat-list-more">
              ...
              >
                show {Math.min(RENDERED_CHATS_PAGE, hiddenActiveChats())} more
                chats · {hiddenActiveChats()} hidden
              </button>
            </li>
          </Show>
            ...
            <Show when={!bag.archivedCollapsed()}>
              ...
              <Show when={hiddenArchivedChats() > 0}>
                <li class="chat-list-more">
                  ...
                  >
                    show {Math.min(RENDERED_CHATS_PAGE, hiddenArchivedChats())}{" "}
                    more archived · {hiddenArchivedChats()} hidden
                  </button>
                </li>
              </Show>
            </Show>
      ...
      <div
        class="sidebar-tab-pair"
        classList={{ "has-chat": Boolean(bag.chatId()) }}
      >
        <button
          type="button"
          class="sidebar-tab"
          classList={{ active: bag.view() === "traces" && !bag.traceChatId() }}
          title="open all traces"
          aria-label="open all traces"
          onClick={() => navigate(() => bag.showTraces(null))}
        >
          ...
          {(currentChatId) => (
            <button
              ...
              classList={{
                active:
                  bag.view() === "traces" &&
                  bag.traceChatId() === currentChatId(),
              }}
              title="open traces for current chat"
              aria-label="open traces for current chat"
              onClick={() => navigate(() => bag.showTraces(currentChatId()))}
            >
              <span class="sidebar-tab-label">for chat</span>
            </button>
          )}
        </Show>
      </div>

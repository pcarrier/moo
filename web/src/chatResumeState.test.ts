...
function step(
  kind: string,
  status: string,
  extra: Partial<Extract<TimelineItem, { type: "step" }>> = {},
): TimelineItem {
  return {
    type: "step",
    step: (extra.step ?? `step-${kind}-${status}`) as Extract<
      TimelineItem,
      { type: "step" }
    >["step"],
    kind,
...
describe("restartable chat state", () => {
  test("offers resume for terminal error rows even when chat summary is done", () => {
    expect(
      hasRestartableConversationState([
  ...
  test("canResumeAgent consults timeline state after terminal summaries", () => {
    ...
    expect(stateSource).toContain('status === "agent:Running"');
    expect(stateSource).toContain('status === "agent:Queued"');
    expect(stateSource).toContain('status === "ui:Pending"');
  });
});

...
describe("header control icons", () => {
  test("uses fixed-size SVG icons for sidebar and app-panel controls", () => {
    ...
    expect(sidebar).toContain("props.bag.rightSidebarMaximized() ? (");
    expect(sidebar).toContain("<RestoreIcon />");
    expect(sidebar).toContain("<MaximizeIcon />");
    expect(chatApps).toContain(
    ...
    expect(
      [headerControls, sidebar, chatApps, timeline].join("\n"),
    ).not.toContain("☰");
  ...
  test("shared icon CSS fixes every toolbar glyph to the same size", () => {
    ...
    const iconRule = css.slice(
      css.indexOf(".ui-icon"),
      css.indexOf(".token-compact-icon"),
    );
  ...
  test("app bridge listens before posting requests", () => {
    ...
    expect(
      requestBlock.indexOf("window.addEventListener('message', onMessage);"),
    ).toBeGreaterThan(-1);
    expect(
  ...
  test("chat burger menu button has no hairline frame", () => {
    const menuRule = css.slice(
      css.indexOf(".chat-menu {"),
      css.indexOf(".chat-action-menu"),
    );
  ...
  test("plus and maximize controls use the smaller header button frame", () => {
    ...
    expect(chatApps).toContain('class="header-icon-button"');
    expect(chatApps).toContain('title="close app"');
    const sharedRule = css.slice(
      css.indexOf(".collapse-btn,"),
      css.indexOf(".mobile-scrim"),
    );
    ...
    const smallControlRule = css.slice(
      css.indexOf(".new-chat-trigger,\n.right-sidebar-size-toggle"),
      css.indexOf(".new-chat-trigger {"),
    );
    expect(css).toContain("--compact-header-button-size: 1.5rem;");
    expect(smallControlRule).toContain(
      "inline-size: var(--compact-header-button-size);",
    );
    expect(smallControlRule).toContain(
      "block-size: var(--compact-header-button-size);",
    );
    expect(smallControlRule).toContain(
      "flex-basis: var(--compact-header-button-size);",
    );

    const rightActionsStart = css.indexOf(".right-sidebar-actions {");
    const rightActionsRule = css.slice(
      rightActionsStart,
      css.indexOf(".right-tabs", rightActionsStart),
    );
    expect(rightActionsRule).toContain(
      "calc((var(--top-bar-h) - var(--compact-header-button-size)) / 2)",
    );
  });
  test("compact icon is a simple legible compression glyph", () => {
    ...
    expect(icons).toContain(
      'd="M5.75 5.25 8 7.5l2.25-2.25M5.75 10.75 8 8.5l2.25 2.25"',
    );

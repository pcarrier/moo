...
describe("PWA installability", () => {
  ...
  test("links install metadata from the document head", () => {
    ...
    expect(indexHtml).toContain(`content="${LIGHT_THEME_COLOR}"`);
    expect(indexHtml).toContain('media="(prefers-color-scheme: light)"');
    expect(indexHtml).toContain(`content="${DARK_THEME_COLOR}"`);
    expect(indexHtml).toContain('media="(prefers-color-scheme: dark)"');
    expect(indexHtml).toContain('rel="icon"');
    expect(indexHtml).toContain('href="/icons/moo.svg"');
  ...
  test("registers only on secure contexts or localhost", () => {
    expect(
      canRegisterServiceWorker(
        { protocol: "https:", hostname: "moo.example" },
        true,
      ),
    ).toBe(true);
    expect(
    ...
    expect(
      canRegisterServiceWorker(
        { protocol: "http:", hostname: "moo.example" },
        false,
      ),
    ).toBe(false);
  });

...
flowchart TD
  ...
  Map --> Chat[chats/messages/export<br/>commands/chats.ts]
...
sequenceDiagram
  ...
  API->>Store: memory triples ui:&lt;id&gt; rdf:type ui:App
  Agent->>API: open(uiId, instanceId?)
  API->>Chat: chat facts ui:involves/ui:primary
  API->>Store: pointer uiinst/&lt;instance&gt;/state
